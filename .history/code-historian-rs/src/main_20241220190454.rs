...
#[derive(Parser)]
#[command(author, version, about = "Code evolution analysis tool", long_about = None)]
struct Cli {
...
enum Commands {
    ...
    /// Analyze repository with detailed options
    Analyze {
        /// Path to analyze (overrides global path)
        path: Option<PathBuf>,
    ...
    Watch {
        /// Path to watch (overrides global path)
        path: Option<PathBuf>,
...
async fn main() -> Result<()> {
    ...
    // Load configuration
    let config = HistorianConfig::load()?;
    ...
    match cli.command.unwrap_or(Commands::Analyze { 
        ...
        Commands::Watch { path, debounce, visualize } => {
            let target_path = path.unwrap_or(repo_path);
            watch_repository(&target_path, &config, debounce, visualize).await?;
        }
    ...
    match command {
        ...
        ConfigCommands::Get { key } => {
            ...
            if let Some(value) = config.get_value(&key) {
                ...
                Err(HistorianError::Config(format!("Unknown config key: {}", key)))
            }
        }
        ConfigCommands::Set { key, value } => {
            let mut config = HistorianConfig::load()?;
...
fn generate_report(output_dir: &PathBuf, analysis: &code_historian::Analysis) -> Result<()> {
    ...
    for (category, count) in &analysis.metrics.category_distribution {
        let percentage = (*count as f64 / total_changes) * 100.0;

...
use crate::{Analysis, Category, Change, Result, HistorianError};
    ...
    pub fn generate_timeline(&self, analysis: &Analysis) -> Result<String> {
        ...
        for (i, change) in analysis.changes.iter().enumerate() {
            ...
            if let Some(prev) = prev_node {
                let edge = edge!(node!(prev) => node!(node_id.clone()));
        ...
        // Generate DOT file
        let dot_string = graph.print(&PrinterContext::default());
        let output_path = format!("{}/timeline.png", self.output_dir);

        // Generate PNG using Graphviz
        graphviz_rust::exec(
            dot_string,
            &[CommandArg::Format("png"), CommandArg::Output(&output_path)],
        ...
        for (i, (category, _)) in categories.iter().enumerate() {
            chart.draw_series(std::iter::once(
                Text::new(
                    category.clone(),
                    (i, -5.0),
                    ("sans-serif", 12).into_font().rotate(45.0),
                ),
            ))?;
        }
